...
def run_agent_query(query: str):
    """Feeds the user query into the agent stream and handles trace parsing cleanly."""
    events = agent_executor.stream(
        {"messages": [("user", query)]},
        stream_mode="values"
    )
...
def interactive_shell():
    """Runs an interactive terminal loop for unhardcoded queries."""
    print("="*60)
    print("INTERACTIVE AGENT SHELL ACTIVATED")
    print("Type your prompt freely. Type 'exit' or 'quit' to stop.")
    print("="*60)
    
    while True:
        try:
            # Dynamically request input from the user in the terminal
            user_input = input("\nEnter your prompt: ").strip()
            
            # Check for exit commands
            if user_input.lower() in ['exit', 'quit']:
                print("\nGoodbye! Exiting agent shell.")
                break
